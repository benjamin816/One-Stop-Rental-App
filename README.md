...
The first visit requires name, email, phone, and contact consent. The separate investor-newsletter opt-in is optional and unchecked; the optional Raleigh investing contact request sends an immediate email to `benjamin.carver@exprealty.com`. A timestamp in `localStorage` skips the gate on later visits from the same browser.

Investor leads go only to the separate **INVESTOR MASTER LEAD SHEET**. The standalone Google Apps Script source is `scripts/investor-lead-apps-script.gs`; it is bound to that sheet and deployed as a Web app. It records the newsletter choice and timestamp separately from calculator/contact consent. Set `VITE_INVESTOR_LEAD_ENDPOINT` to the Web app `/exec` URL when building. The gate stays locked if the endpoint is missing or does not acknowledge a saved lead.
