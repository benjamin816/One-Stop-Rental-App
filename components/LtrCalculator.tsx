...
const LtrCalculator: React.FC<LtrCalculatorProps> = ({ data, rider, sellerCredit, onRiderChange, onSellerCreditChange, onChange, onCheckboxChange, onPushData, onExportPdf, showSaveButton, onSave }) => {
  ...
  const metrics = useMemo(() => {
    ...
    const refiCf = data.rent - refiPiti - opex;
  ...
  const payoffRevenue = data.rent + (rider.enabled ? riderImpact.monthlyRevenue : 0);
  ...
  return (
    <div id={CALCULATOR_ID} className="bg-white rounded-2xl shadow-lg p-5">
      ...
      <div className={`${ACTIONS_CLASS} flex justify-end items-center gap-2 mb-4`}>
        ...
        <button
          onClick={() => onExportPdf(CALCULATOR_ID, 'LTR_Analysis', ACTIONS_CLASS)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="col-span-1 md:col-span-2">
          <SellerCreditModule idPrefix="ltr" state={sellerCredit} result={sellerCreditResult} onChange={onSellerCreditChange} />
        </div>

        <SectionHeader>The Loan</SectionHeader>
        <InputField label="Rate (%)" id="ltr_rate" value={data.rate} onChange={e => onChange('rate', e.target.value)} min={0} max={15} step={0.001} decimalPlaces={3} />
      ...
      <NewConstructionRider idPrefix="ltr" rider={rider} assumptions={riderAssumptions} onChange={onRiderChange} />
      ...
      <PayoffAnalysis
        ...
        note="Shows the projected cash flow once the loan balance is paid off. Taxes, insurance, operating expenses, and enabled rider operations remain included."
      />
